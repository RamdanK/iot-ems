...
class DevicePowerTrendChart extends ApexChartWidget
{
     ...
     * @var string
     */
    protected static ?string $chartId = 'device-power-chart';
     ...
     * @var string|null
     */
    protected static ?string $heading = 'Power Trend';
    ...
    {
        $data1 = $event['deviceStatus']['power1'];
        $data2 = $event['deviceStatus']['power2'];
    ...
    {
        $data = DeviceMatrixService::getDeviceTrend($this->deviceId, ['created_at', 'power1', 'power2']);
        if ($data->count() > 0) {
            $this->data['data'] = [
                [
                    'name' => 'Power 1',
                    'data' => $data->map(fn (DeviceStatus $value) => (float) $value->power1)->toArray(),
                ],
                [
                    'name' => 'Power 2',
                    'data' => $data->map(fn (DeviceStatus $value) => (float) $value->power2)->toArray(),
                ],
            ];
