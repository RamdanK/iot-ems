...
class DeviceBatteryTrendChart extends ApexChartWidget
{
     ...
     * @var string
     */
    protected static ?string $chartId = 'device-battery-chart';
     ...
     * @var string|null
     */
    protected static ?string $heading = 'Battery Level Trend';
    ...
    {
        $data1 = $event['deviceStatus']['battery'];
    ...
    {
        $data = DeviceMatrixService::getDeviceTrend($this->deviceId, ['created_at', 'battery']);
        if ($data->count() > 0) {
            $this->data['data'] = [
                [
                    'name' => 'Battery',
                    'data' => $data->map(fn(DeviceStatus $value) => (float) $value->battery)->toArray(),
                ],
            ];
